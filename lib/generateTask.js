...
var DynamicTask = function () {
    ...
    _createClass(DynamicTask, [{
        ...
        value: function setup(ctx, gulp) {
            var name = this.factory(ctx, this.getInfo(), gulp || coregulp);
            if (name) {
    ...
    _.each(_.isArray(tasks) ? tasks : [tasks], function (dt) {
        ...
        if (dt.watch && match && match.oper & TaskConfig_1.Operation.watch) {
            taskseq.push(createWatchTask({
                ...
                name: function name(ctx) {
                    return ctx.subTaskName(dt) + '-twatch';
                },
                watchTasks: function watchTasks(ctx) {
                    return [ctx.subTaskName(dt)];
                }
            }));
        }
    });
...
function createCustomTask(dt) {
    var factory = function factory(ctx, info, gulp) {
        var tk = ctx.subTaskName(info);
