    ...
    _.each(_.isArray(tasks) ? tasks : [tasks], function (dt) {
        ...
        if (dt.watch && match && match.oper & TaskConfig_1.Operation.watch) {
            taskseq.push(createWatchTask({
                ...
                name: function name(oper, env) {
                    return utils_1.taskStringVal(dt.name, oper, env) + '-watch';
                },
                watchTasks: function watchTasks(ctx) {
                    return [utils_1.taskStringVal(dt.name, ctx.oper, ctx.env)];
                }
            }));
        }
    });
...
function createWatchTask(dt) {
    var factory = function factory(ctx, gulp) {
        var watchs = _.isFunction(dt.watchTasks) ? dt.watchTasks(ctx, dt) : dt.watchTasks;
